...
goog.provide('org.jboss.core.visualization.Histogram');
 ...
 */
org.jboss.core.visualization.Histogram = function(element) {
   ...
   * @private
   */
  this.log_ = goog.debug.Logger.getLogger('org.jboss.core.visualization.Histogram');
};
goog.inherits(org.jboss.core.visualization.Histogram, goog.events.EventTarget);


/** @inheritDoc */
org.jboss.core.visualization.Histogram.prototype.disposeInternal = function() {
  org.jboss.core.visualization.Histogram.superClass_.disposeInternal.call(this);
 ...
 */
org.jboss.core.visualization.Histogram.prototype.initialize = function(css_class, w, h, opt_m) {
 ...
 * @private
 */
org.jboss.core.visualization.Histogram.prototype.brushEnd_ = function() {
  var extent = this.brush.extent();
  if (goog.isDateLike(extent[0]) && goog.isDateLike(extent[1])) {
    this.dispatchEvent(
        new org.jboss.core.visualization.IntervalSelected(
 ...
 * @private
 */
org.jboss.core.visualization.Histogram.prototype.brush_ = function() {
  var extent = this.brush.extent();
  if (goog.isDateLike(extent[0]) && goog.isDateLike(extent[1])) {
    this.dispatchEvent(
        new org.jboss.core.visualization.IntervalSelected(
...
/**
 * The second parameter can be any value supported
 * by d3 Time-Intervals {@see https://github.com/mbostock/d3/wiki/Time-Intervals}.
 * As of writing it supports: "hour", "day", "week", "month" and "year" (plus other).
 * If the interval is "quarter" then a "month" is used instead.
 *
 * @param {Array.<{time: number, count: number}>} data
 * @param {string} interval
 */
org.jboss.core.visualization.Histogram.prototype.update = function(data, interval) {
  ...
  this.title.text(function() {
    return 'Matching contribution updates by ' +
        (goog.string.endsWith(timeInterval, 's') ? timeInterval : timeInterval + 's');
  });
